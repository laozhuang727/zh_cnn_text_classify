...
def load_multi_classes_files(data_dir):
    """
    Loads MR polarity data from files, splits the data into words and generates labels.
    Returns split sentences and labels.
    """
    ...
    for each in classes:
        ...
        for ii, file in enumerate(files, 1):
            ...
            a_class_samples = read_and_clean_zh_file(class_path + "/" + file)
            x_text.append(a_class_samples)

            # Generate labels
            a_class_labels = [each for _ in a_class_samples]
            labels.append(a_class_labels)

    x_text = np.concatenate(x_text, 0)
    ...
    return [x_text, y, label_binarizer]
